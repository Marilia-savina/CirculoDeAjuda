...
app.get('/', (req, res) => {
    res.send(`
        ...
        <p>Endpoint que leva a uma rota para registrar um pedido de ajuda: /ajuda</p>
        </body>
        `)
})
...
const {nome, endereco, telefone, vagas_total, vagas_disponiveis, aceita_pets} = req.body
const db = await banco()
await db.run(`
    INSERT INTO abrigos(nome, endereco, telefone, vagas_total, vagas_disponiveis, aceita_pets) VALUES (?,?,?,?,?,?)`, [nome, endereco, telefone, vagas_total, vagas_disponiveis, aceita_pets])
